...
namespace ZXing.Encoders.OneD
{
    ...
    /// </summary>
    public abstract class OneDimensionalCodeEncoder : IEncoder
    {
        ...
        public virtual BitMatrix Encode(string contents, BarcodeFormat format, int width, int height, IDictionary<EncodeHintType, object> hints)
        {
            ...
            {
                throw new ArgumentException("Found empty contents", nameof(contents));
            }

            if (width < 0 || height < 0)
            {
                throw new ArgumentOutOfRangeException($"Negative size is not allowed. Input: {width}x{height}");
            }

            var sidesMargin = DefaultMargin;
            if (hints != null)
            {
                var sidesMarginInt = hints.ContainsKey(EncodeHintType.MARGIN) ? hints[EncodeHintType.MARGIN] : null;
                if (sidesMarginInt != null)
                {
                    sidesMargin = sidesMarginInt is int ? (int)sidesMarginInt : int.TryParse(sidesMarginInt.ToString(), out sidesMargin) ? sidesMargin : DefaultMargin;
                }
            }
        ...
        //TODO: return bool instead of throw
        /// <summary>
        ...
        protected static void CheckNumeric(string contents)
        {
            ...
            {
                throw new ArgumentException("Input should only contain digits 0-9", nameof(contents));
            }
        }
        ...
        public abstract BitMatrix Encode(string contents, int width, int height);
        public abstract BitMatrix Encode(string contents, int width, int height, IDictionary<EncodeHintType, object> hints);
    }
}
