...
namespace ZXing.Encoders.OneD.Test
{
    public class ITFWriterTestCase
    {
        ...
        [TestOf(typeof(ITFEncoder))]
        public void TestEncode(string input, string expected)
        {
            var result = new ITFEncoder().Encode(input, BarcodeFormat.ITF, 0, 0);
        ...
        {
            Assert.Throws<ArgumentException>(() => new ITFEncoder().Encode("00123456789abc", BarcodeFormat.ITF, 0, 0));
        }
    }
}